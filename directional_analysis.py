...
class DirectionalAnalysis():
	...
	def angleFit(self, nbins, seg_size, num_events, it_num, save_output=True, save_plots=True, output_path="fits/", show_plots=False, debug=False):
		...
		for th in range(-data_range,data_range):# TODO: do these reference binnings once and store. Recall binned matrices to do FND calculations for fits
			angles.append(th)
	...
	def resolutionPlot(self, n_iters, f_name, write_outputs=True, keep_plots=True, show_plots=False, debug_msg=False):
		...
		for k, sz in enumerate(self.seg_sz):
			...
			for j, num in enumerate(n_evts):
				...
				for i in range(n_iters):
					angle, n_caps = self.angleFit(self.grid_size[k], sz, num, i, write_outputs, keep_plots, self.out_file_path + dir_str[num], False, False)
...
da.resolutionPlot(itnums, plot_name)
sys.exit()
